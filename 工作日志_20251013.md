...
- 原因: LIMIT/OFFSET 参数化处理问题
- 解决: 改用模板字符串直接嵌入已验证的参数
- 文件: `backend/src/models/coupon.model.ts`
- 状态: ✅ 已修复
...
- 原因: 字符编码配置不完整
- 解决方案:
  1. MySQL 连接添加 charset: 'utf8mb4'
  2. 添加响应头 Content-Type: application/json; charset=utf-8
  3. 使用 SET NAMES utf8mb4 重新插入数据
- 文件: 
...
- ✅ 数据插入: SET NAMES utf8mb4
- ✅ API 响应: charset=utf-8

**测试数据**:
创建了3张测试优惠券:
1. 新用户欢迎券 (WELCOME50) - 满200减50
2. 8折优惠券 (DISCOUNT20) - 全场8折
3. 无门槛券 (GIFT20) - 直接减20
...
**新增文档**:
- ✅ COUPON_FRONTEND_GUIDE.md - 优惠券前端功能指南
- ✅ PROFILE_PAGE_GUIDE.md - 个人中心使用指南
- ✅ test-coupon-frontend.sh - 自动化测试脚本

**更新日志**:
- ✅ 开发日志_前端.md - 详细记录前端实现
- ✅ 开发日志_后端.md - 详细记录后端修复
- ✅ 工作日志_20251013.md - 本次工作记录
...
sql: 'SELECT * FROM coupons ... LIMIT ? OFFSET ?'
```

**分析**:
- MySQL prepared statement 对 LIMIT/OFFSET 参数处理有限制
- 参数化可能导致类型错误
...
// 修复前 ❌
LIMIT ? OFFSET ?
params: [...queryParams, page_size, offset]
...
| 数据库表字符集 | ✅ utf8mb4 | 正确 |
| 数据存储 | ❌ 乱码 | 问题所在 |
| MySQL 连接 | ❌ 未配置 | 缺少 charset |
| API 响应头 | ❌ 未指定 | 缺少 charset |
...
**步骤3**: 重新插入数据
```bash
docker exec -i ecommerce-mysql bash << 'BASH'
mysql -uroot -proot123456 ecommerce << 'SQL'
SET NAMES utf8mb4;
DELETE FROM coupons;
...
**验证结果**:
```bash
curl http://localhost:3001/api/coupons/available | jq '.data[0].name'
# 输出: "新用户欢迎券" ✅ 正确显示
```
...
1. **导入路径错误**: 
   ...
   - 正确: `import { useAuthStore } from '@/store/useAuthStore'`
...
实现了从数据库到前端的全链路 UTF-8 编码：

```
数据库表 (utf8mb4)
    ↓
MySQL连接 (charset='utf8mb4')
    ↓
数据插入 (SET NAMES utf8mb4)
    ↓
数据查询 (自动使用连接字符集)
    ↓
API响应 (Content-Type: charset=utf-8)
    ↓
前端显示 (正确的中文) ✅
```
...
**颜色系统**:
- 用户信息: 蓝色渐变 (from-blue-600 to-blue-800)
- 优惠券: 橙红色渐变 (from-orange-500 to-red-500)
- 会员专享: 紫粉色渐变 (from-purple-500 to-pink-500)
...
```typescript
try {
  const result = await CouponModel.getList(params);
...
```bash
# 前端日志
docker logs ecommerce-frontend --tail 100 -f

# 后端日志
docker logs ecommerce-backend --tail 100 -f | grep -E "(error|优惠券)"
...
# 查看数据
SELECT coupon_id, name, code, type, discount_value 
FROM coupons;
...
- 🎓 深入理解字符编码机制
- 🎓 掌握 MySQL prepared statement 特性
- 🎓 学习前端构建和部署流程
- 🎓 提升 UI/UX 设计能力
- 🎓 增强问题诊断和调试技巧
...
**管理后台**:
- 优惠券管理: http://localhost:3000/admin/coupons

**API 文档**:
- 用户端 API: http://localhost:3001/api/coupons
- 管理端 API: http://localhost:3001/api/admin/coupons

**文档**:
- 前端指南: COUPON_FRONTEND_GUIDE.md
- 个人中心指南: PROFILE_PAGE_GUIDE.md
